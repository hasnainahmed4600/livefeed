...
class TermsOfServiceScreen extends StatelessWidget {
  static Route route() {
    return MaterialPageRoute<void>(builder: (_) => TermsOfServiceScreen());
  }
  ...
  Widget build(BuildContext context) {
    final appBar = LivefeedAppBar(
      ...
      onHowItWorksTapped: () => Navigator.of(context).pushReplacement(
          HowItWorksScreen.route(previousRoute: TermsOfServiceScreen.route())),
      onMarketplaceTapped: () =>
          Navigator.of(context).pushReplacement(MarketplaceScreen.route(
        previousRoute: TermsOfServiceScreen.route(),
      )),
    );
    return Scaffold(
      appBar: appBar.build(),
      body: TermsOfServiceLayout(),
    );
  }
}
