...
class AddContentCard extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _AddContentCardState();
}

class _AddContentCardState extends State<AddContentCard> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<PostFormBloc, PostFormState>(
      builder: (context, state) {
        ...
        final uploadedVideoPaths = loadedState.selectedMedia.value
            .where((media) => media.isVideo)
            .map((e) => e.file.path)
            .toList();
        final uploadedImagePaths = loadedState.selectedMedia.value
            .where((media) => !media.isVideo)
            .map((e) => e.file.path)
            .toList();

        if (loadedState.headline.value.isNotEmpty &&
            loadedState.description.value.length < 100 &&
            uploadedVideoPaths.isNotEmpty) suggestedPrice = '\$290-590';
        if (loadedState.headline.value.isNotEmpty &&
            loadedState.description.value.length > 100 &&
            uploadedVideoPaths.isNotEmpty) suggestedPrice = '\$390-790';
        if (loadedState.headline.value.isNotEmpty &&
            loadedState.description.value.length < 100 &&
            uploadedImagePaths.isNotEmpty) suggestedPrice = '\$190-290';
        if (loadedState.headline.value.isNotEmpty &&
            loadedState.description.value.length > 100 &&
            uploadedImagePaths.isNotEmpty) suggestedPrice = '\$290-490';
        if (loadedState.headline.value.isNotEmpty &&
            loadedState.description.value.isNotEmpty &&
            uploadedImagePaths.length > 1) suggestedPrice = '\$390-590';
        if (loadedState.headline.value.isNotEmpty &&
            loadedState.description.value.length > 100 &&
            uploadedImagePaths.length > 1 &&
            uploadedVideoPaths.isNotEmpty) suggestedPrice = '\$490-890';
            ...
            child: Column(
              ...
              children: [
                ...
                MediaUploadField(
                  ...
                  onFileDeleted: (fileId) => context
                      .read<PostFormBloc>()
                      .add(MediaFileRemoved(fileId)),
                ),
                const Padding(padding: EdgeInsets.only(top: 10)),
                LocationField((newAddress, newZipCode) {
                  ...
                  context
                      .read<PostFormBloc>()
                      .add(LocationUpdated('$newAddress $zipCode'));
                }),
                // _LocationControl(),
                // PostAddressControl(
                ...
                if (suggestedPrice.isNotEmpty)
                  Text(
                    'Estimated lease price: $suggestedPrice',
              ...
              trackBar: FlutterSliderTrackBar(
                centralWidget: Text(
                  loadedState.status.isValidated
                      ...
                      : 'Fill out all the fields and add at least 1 photo or video',
                  textAlign: TextAlign.center,
                  style: LiveFeedTheme.theme.textTheme.bodyText1.copyWith(
...
class _SocialMediaFooter extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Row(
